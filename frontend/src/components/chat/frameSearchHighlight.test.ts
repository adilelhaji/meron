...
describe('matchRanges', () => {
  ...
  it('still folds the case of a character that expands', () => {
    ...
    expect(hits).toBe(1)
    expect([...ranges]).toEqual([[0, [[0, 1]]]])
  })

  it('folds ordinary case in both directions', () => {
    expect([...matchRanges(['straße'], 'STRAßE').ranges]).toEqual([[0, [[0, 6]]]])
    expect([...matchRanges(['ÉCOLE'], 'école').ranges]).toEqual([[0, [[0, 5]]]])
  })
  ...
  it('matches a Greek sigma in either of its lowercase forms', () => {
    ...
    expect([...matchRanges(['ΟΣ'], 'ΟΣ').ranges]).toEqual([[0, [[0, 2]]]])
    expect([...matchRanges(['ΟΣ'], 'ος').ranges]).toEqual([[0, [[0, 2]]]])
    expect([...matchRanges(['Ος'], 'ΟΣ').ranges]).toEqual([[0, [[0, 2]]]])
  })
